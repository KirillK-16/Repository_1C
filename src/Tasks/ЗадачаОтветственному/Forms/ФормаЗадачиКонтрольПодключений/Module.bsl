	...
	Если ИмяМакета <> Неопределено Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				...
				Метаданные.Задачи.ЗадачаОтветственному.Формы.ФормаЗадачиКонтрольПодключений,
				,
				Комментарий);
...
&НаКлиенте
Процедура ЛожнаяТревога(Команда)
	
	ОбработатьЛожнуюТревогу();
	ЭтотОбъект.Закрыть();
	
КонецПроцедуры

&НаСервере
Процедура ОбработатьЛожнуюТревогу()
	
	ЗаписьОЛожномСрабатывании = РегистрыСведений.ЛожныеТревогиКонтрольПодключений.СоздатьМенеджерЗаписи();
	ЗаписьОЛожномСрабатывании.КонтрольнаяПроцедура = Объект.ИсточникЗадачи;
	ЗаписьОЛожномСрабатывании.Записать(Истина);
	
	ТекстПолучения = "Ложная тревога при контроле подключений";
