...
Процедура ОбновитьРейтинг()
	...
	|	КОЛИЧЕСТВО(ТехнологическийЖурнал.ПоследняяСтрокаКонтекста) КАК КоличествоИсключенийСДаннымТекстом,
	|	ТехнологическийЖурнал.ПоследняяСтрокаКонтекста КАК ПоследняяСтрокаКонтекста
	|ИЗ
	|	РегистрСведений.ТехнологическийЖурнал КАК ТехнологическийЖурнал
	|	%Фильтр
	|СГРУППИРОВАТЬ ПО
	|	ТехнологическийЖурнал.ПоследняяСтрокаКонтекста
	|УПОРЯДОЧИТЬ ПО КоличествоИсключенийСДаннымТекстом УБЫВ";
	...
	ТекстыИгнорируемыхОшибок = Новый Массив;
	Для Каждого СтрокаТаблицы Из ИгнорируемыеОшибки Цикл
		ТекстыИгнорируемыхОшибок.Добавить(СтрокаТаблицы.ТекстИсключения);
	КонецЦикла;
	
	Если ТекстыИгнорируемыхОшибок.Количество() > 0 Тогда
		Фильтр = Фильтр + " И ТехнологическийЖурнал.ПоследняяСтрокаКонтекста НЕ В (&ТекстыИгнорируемыхОшибок) ";
		ПараметрыЗапроса.Вставить("ТекстыИгнорируемыхОшибок", ТекстыИгнорируемыхОшибок);
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		СтрокаРейтинга.ПоследняяСтрокаКонтекста = СокрЛП(Выборка.ПоследняяСтрокаКонтекста);
	КонецЦикла;
...
Функция КлючНастроек()
	Возврат "НастройкиФормыКонтрольИсключенийПоТехнологическомуЖурналуПоПоследнейСтроке";
КонецФункции
...
Процедура ИгнорироватьИсключение(Команда)
	...
	ИгнорироватьИсключениеСервер(ТекущееИсключение.ПоследняяСтрокаКонтекста);
КонецПроцедуры

&НаСервере
Процедура ИгнорироватьИсключениеСервер(ТекстИсключения)
