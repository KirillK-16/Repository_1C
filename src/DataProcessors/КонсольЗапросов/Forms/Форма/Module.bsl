...
Процедура ЗадатьУникальныйИдентификатор(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеВводаУникальногоИдентификатора", ЭтотОбъект, НомерСтроки);
	УникальныйИдентификаторСтрокой = "";
	ПоказатьВводСтроки(ОписаниеОповещения, УникальныйИдентификаторСтрокой, НСтр("ru = 'Введите уникальный идентификатор'"));
КонецПроцедуры
...
Процедура ОткрытьКонструкторЗапроса(Команда)
	...
	Если ВерсияКонфигурации835ИБольше() Тогда
		#Если ТолстыйКлиентУправляемоеПриложение ИЛИ ТолстыйКлиентОбычноеПриложение ИЛИ ТонкийКлиент Тогда
			...
			КонецЕсли;
			ОписаниеОповещения = Новый ОписаниеОповещения("ПослеЗакрытияКонструктораЗапроса", ЭтотОбъект, ИндексТекущегоЗапроса);
...
Процедура ЗапуститьВыполнениеЗапроса(ВыводитьВременныеТаблицы)
	...
	ВыполнитьЗапросСервер(ИндексТекущегоЗапроса, РезультатЗапроса, ВыводитьВременныеТаблицы, ВыводитьИдентификатор, Текст, ТекстСообщения, ДанныеФЗ);
	Если Фоновое Тогда
		...
		ЭтотОбъект.РезультатЗапроса.Область("R3C1").Текст = "Начало формирования: " + ДатаНачалаФормирования;
...
//
&НаСервере
Процедура ВыполнитьЗапросСервер(ИндексТекущегоЗапроса, ТабличныйДокументРезультата, ВыводитьВременныеТаблицы, ВыводитьИдентификатор, ТекстЗапроса, ТекстСообщения, ДанныеФЗ)
	...
	Если Фоновое Тогда
		...
		ПараметрыЭкспортнойПроцедуры.Добавить(Текст);
		МассивПараметров = Объект.Параметры.Выгрузить(Объект.Параметры.НайтиСтроки(Фильтр));
		ПараметрыЭкспортнойПроцедуры.Добавить(МассивПараметров);
		ПараметрыЭкспортнойПроцедуры.Добавить(ТабличныйДокументРезультата);
		ПараметрыЭкспортнойПроцедуры.Добавить(ПараметрыВыводаЗапроса);
		ПараметрыЭкспортнойПроцедуры.Добавить(ОтчетПоВыполнениюЗапроса);
		ПараметрыЭкспортнойПроцедуры.Добавить(МеткаЗапроса);
		ПараметрыЭкспортнойПроцедуры.Добавить(АдресХранилища);
		
		Ключ = ЭтотОбъект.УникальныйИдентификатор;
		
		ПараметрыЗадания = Новый Массив;
		ИмяЭкспортнойПроцедуры = "Обработки.КонсольЗапросов.ВыполнитьЗапрос";
		...
		ДанныеФЗ.Вставить("ИндексТекущегоЗапроса", ИндексТекущегоЗапроса);
...
&НаСервере
Процедура ПослеВыполнитьЗапросСервер(РезультатФЗ, ИндексТекущегоЗапроса)
	ТекущийЗапрос = Объект.Запросы.Получить(ИндексТекущегоЗапроса - 1);
	МассивЗапросов = РезультатФЗ.МассивЗапросов;
	ТабличныйДокументРезультата = РезультатФЗ.ТДРезультатаЗапроса;
	ОтчетПоВыполнениюЗапроса = РезультатФЗ.ОтчетПоВыполнениюЗапроса;
	Если ЗначениеЗаполнено(ОтчетПоВыполнениюЗапроса.ТекстСообщения) Тогда
		ПоказатьСообщениеПользователю(ОтчетПоВыполнениюЗапроса.ТекстСообщения);
	КонецЕсли;
	...
	РезультатЗапроса = ТабличныйДокументРезультата;
	// Заполнение адреса временного хранилища для результата.
	ТекущийЗапрос.АдресРезультата = ПоместитьВоВременноеХранилище(ТабличныйДокументРезультата, УникальныйИдентификатор);
	ТекущийЗапрос.ВремяВыполнения = ОтчетПоВыполнениюЗапроса.ВремяВыполнения;
	ТекущийЗапрос.КоличествоСтрок = ОтчетПоВыполнениюЗапроса.КоличествоСтрок;
...
Процедура ПроверкаВыполненияФоновогоЗадания()
	...
	ЭтотОбъект.РезультатЗапроса.Область("R4C1").Текст = "Длительность формирования: " + Формат(ТекущаяДата() - ДатаНачалаФормирования, "ЧН=0") + " сек";
	...
	Если РезультатПроверки.Состояние <> "Активно" Тогда
        ...
        ИначеЕсли РезультатПроверки.Состояние = "Отменено" Тогда
			ЭтотОбъект.РезультатЗапроса.Область("R2C1").Текст = "Отменено пользователем...";
			ЭтотОбъект.РезультатЗапроса.Область("R3C1").Текст = "";
			ЭтотОбъект.РезультатЗапроса.Область("R4C1").Текст = "";
...
Функция ПроверкаВыполненияФоновогоЗаданияНаСервере(Параметры)
	...
	ИначеЕсли ФЗ.Состояние = СостояниеФоновогоЗадания.Завершено Тогда
		...
		СтрРезультат = ПолучитьИзВременногоХранилища(Параметры.АдресХранилища); 
		ПослеВыполнитьЗапросСервер(СтрРезультат, Параметры.ИндексТекущегоЗапроса);
...
Процедура СохранитьФайлЗапросаЗавершениеНачатьПодключениеРасширенияРаботыСФайламиЗавершение(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если Подключено Тогда
		...
		//Если ПустаяСтрока(ИмяФайла) Тогда
			...
			ОписаниеОповещения = Новый ОписаниеОповещения("СохранитьФайлЗапросаЗавершениеНачатьПодключениеРасширенияРаботыСФайламиЗавершениеДиалогПоказатьЗавершение", ЭтотОбъект, ДополнительныеПараметры);
			Диалог.Показать(ОписаниеОповещения);
		//Иначе
		//	СохранитьФайлЗапросаЗавершениеНачатьПодключениеРасширенияРаботыСФайламиЗавершениеОбщая(ИмяФайла, ДополнительныеПараметры);		
		//КонецЕсли;
	Иначе
...
&НаКлиенте
Процедура СохранитьФайлЗапросаЗавершениеНачатьПодключениеРасширенияРаботыСФайламиЗавершениеОбщая(ИмяФайла, Оповещение)
	Если Не ПустаяСтрока(ИмяФайла) Тогда 
		ДвоичныеДанные = СохранитьЗапросыСервер();
		ДвоичныеДанные.Записать(ИмяФайла);
...
Процедура ОбработкаЧтенияФайла(Удалять)
	...
	Диалог.ПроверятьСуществованиеФайла  = Истина;
	Диалог.МножественныйВыбор			= Ложь;
	ОписаниеОповещения = Новый ОписаниеОповещения("ОбработкаЧтенияФайлаЗавершение", ЭтотОбъект, Удалять);
	Диалог.Показать(ОписаниеОповещения);
