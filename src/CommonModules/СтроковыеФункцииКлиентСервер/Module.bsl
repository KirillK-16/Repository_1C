...
Функция СтрокаНачинаетсяС(Строка, СтрокаПоиска) Экспорт 
	...
	Возврат СтрокаПоиска = Лев(Строка, ДлинаСтроки);	
	
КонецФункции

Функция СтрокаЗаканчиваетсяНа(Строка, СтрокаПоиска) Экспорт

	ДлинаСтроки = СтрДлина(СтрокаПоиска);
	
	Возврат СтрокаПоиска = Прав(Строка, ДлинаСтроки);
...
Функция РазложитьСтрокуПоСловам(Знач Строка) Экспорт
	...
	Для ПозицияСимвола = 1 По СтрДлина(Строка) Цикл
        ...
        Если ПозицияСимвола <> 1 Тогда
            ПредСимвол = Сред(Строка, ПозицияСимвола - 1, 1);
            ПредСимволВРег = ПредСимвол = ВРег(ПредСимвол);
            Если ВРег(ПредСимвол) = "И" Тогда
                ПредСимволВРег = Ложь;
            КонецЕсли;
        Иначе
            ПредСимволВРег = Ложь;
		...
		Если НЕ ПредСимволВРег И ТекСимвол = ВРег(ТекСимвол) И (ТолькоКириллицаВСтроке(ТекСимвол) ИЛИ ТолькоЛатиницаВСтроке(ТекСимвол)) Тогда
