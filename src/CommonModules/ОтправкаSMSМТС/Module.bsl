	...
	Для Каждого Элемент Из НомераПолучателей Цикл
		...
		Если Не ПустаяСтрока(НомерПолучателя) Тогда
			Попытка
				...
				Результат.ПараметрыОтправки.Добавить(Новый Структура("НомерПолучателя,ИдентификаторСообщения",	
			...
			Исключение
				...
				Результат.ОписаниеОшибки = Результат.ОписаниеОшибки 
										 + НСтр("ru = 'SMS на номер'") + " " + Элемент 
										 + НСтр("ru = 'не отправлено'")
										 + ": "
...
Функция СтатусДоставкиSMS(СтатусСтрокой)
	...
	Возврат ?(Результат = Неопределено, Перечисления.СтатусыДоставкиSMS.Новое, Результат);
КонецФункции

Функция ПодключитьВебСервис()
	Возврат WSСсылки.ОтправкаSMSМТС.СоздатьWSПрокси("http://mcommunicator.ru/M2M", "MTS_x0020_Communicator_x0020_M2M_x0020_XML_x0020_API", "MTS_x0020_Communicator_x0020_M2M_x0020_XML_x0020_APISoap12",,60); 
КонецФункции
...
Процедура ВыполнитьОперациюСФункциейF(a, b, c, d, X, s, t)
	ВыполнитьОперацию(ВнутрПобитовоеИли(ВнутрПобитовоеИ(b, c), ВнутрПобитовоеИ(ВнутрПобитовоеНе(b), d)), a, b, X, s, t);
КонецПроцедуры

Процедура ВыполнитьОперациюСФункциейG(a, b, c, d, X, s, t)
	ВыполнитьОперацию(ВнутрПобитовоеИли(ВнутрПобитовоеИ(b, d), ВнутрПобитовоеИ(ВнутрПобитовоеНе(d), c)), a, b, X, s, t);
КонецПроцедуры
...
Процедура ВыполнитьОперациюСФункциейI(a, b, c, d, X, s, t)
	ВыполнитьОперацию(ПобитовоеИсключающееИли(ВнутрПобитовоеИли(ВнутрПобитовоеНе(d), b), c), a, b, X, s, t);
КонецПроцедуры
...
Функция ВнутрПобитовоеНе(БинарныйМассив)
	Результат = Новый Массив;
