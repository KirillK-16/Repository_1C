...
Функция ПодготовитьЗапускВыполненияВФоне(ДополнительныеПараметры = Неопределено, ПараметрыХранилищаНастроек = Неопределено) Экспорт
    
    ПараметрыЗапуска = Новый Массив;
    
    Если ПараметрыХранилищаНастроек = Неопределено Тогда
        ПараметрыЗапускаСтруктура = Новый Структура("ДополнительныеПараметры", ДополнительныеПараметры);
    Иначе
        ПараметрыЗапускаСтруктура = Новый Структура("ДополнительныеПараметры, ПараметрыХранилищаНастроек", ДополнительныеПараметры, ПараметрыХранилищаНастроек);
    КонецЕсли;
    
    ПараметрыЗапуска.Добавить(ПараметрыЗапускаСтруктура);
    
    Возврат ПараметрыЗапуска;
    
КонецФункции

Функция ПодготовитьЗапускВыполненияВФонеВнешняяОбработка(ИмяФайлаВнешнейОбработки, ДополнительныеПараметры, ПараметрыХранилищаНастроек = Неопределено) Экспорт
    
    ПараметрыЗапуска = Новый Массив;
    
    Если ПараметрыХранилищаНастроек = Неопределено Тогда
        ПараметрыЗапускаСтруктура = Новый Структура("ИмяФайлаВнешнейОбработки, ДополнительныеПараметры", ИмяФайлаВнешнейОбработки, ДополнительныеПараметры);
    Иначе
        ПараметрыЗапускаСтруктура = Новый Структура("ИмяФайлаВнешнейОбработки, ДополнительныеПараметры, ПараметрыХранилищаНастроек", ИмяФайлаВнешнейОбработки, ДополнительныеПараметры, ПараметрыХранилищаНастроек);
    КонецЕсли;
            
    ДвоичныеДанныеОбработка = Новый ДвоичныеДанные(ИмяФайлаВнешнейОбработки);
    ПараметрыЗапускаСтруктура.Вставить("ДвоичныеДанныеОбработка", ДвоичныеДанныеОбработка);
