...
Функция СоздатьЭлементПоНаименованию(Наименование, ХешСумма) Экспорт
	...
	Если Результат.Пустой() Тогда
		Попытка
			...
			ЭлементБлокировки.УстановитьЗначение("Объект", "Справочник.ОбластиБизнесСтатистики." + Наименование);
			ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
			БлокировкаДанных.Заблокировать();
		Исключение
			ВызватьИсключение "Центр мониторинга. Ожидаемая управляемая блокировка при создании элемента справочника 'ОбластиБизнесСтатистики'";
		КонецПопытки;
		
		Результат = РезультатЗапросаПоХешСумма(ХешСумма);
		Если Результат.Пустой() Тогда
			НовыйЭлемент = Справочники.ОбластиБизнесСтатистики.СоздатьЭлемент();
...
Функция РезультатЗапросаПоХешСумма(ХешСумма)
	...
	|	Ссылка
	|ИЗ
	|	Справочник.ОбластиБизнесСтатистики
	|ГДЕ
