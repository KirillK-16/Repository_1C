...
Функция СоздатьЭлементПоНаименованию(Наименование) Экспорт
	...
	Если Результат.Пустой() Тогда
		Попытка
			...
			ЭлементБлокировки.УстановитьЗначение("Объект", "Справочник.КлассыОшибокЦентрМониторинга." + Наименование);
			ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
			БлокировкаДанных.Заблокировать();
		Исключение
			ВызватьИсключение "Центр мониторинга. Ожидаемая управляемая блокировка при создании элемента справочника 'КлассыОшибокЦентрМониторинга'";
		КонецПопытки;
		
		Результат = РезультатЗапросаПоНаименованию(Наименование);
		Если Результат.Пустой() Тогда
			НовыйЭлемент = Справочники.КлассыОшибокЦентрМониторинга.СоздатьЭлемент();
...
Функция РезультатЗапросаПоНаименованию(Наименование)
	...
	|	Ссылка
	|ИЗ
	|	Справочник.КлассыОшибокЦентрМониторинга
	|ГДЕ
