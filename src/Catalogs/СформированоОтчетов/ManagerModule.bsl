...
Функция ПолучитьМассивСтруктурыПоказателей(ТипПоказателя, ИдентификаторНастройки, МассивСтруктурПоказателей, ТолькоВыбранные) Экспорт
	...
	Для Каждого СтруктураПоказатель Из МассивСтруктурПоказателей Цикл
		...
		Если ЗначениеЗаполнено(СтруктураПоказатель.Показатель) И СтруктураПоказатель.ТипПоказателя = "СформированоОтчетов" Тогда
	...
	СтруктураДанныхГруппы.Вставить("Код", "Группы отчетов");
	...
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СформированоОтчетов.ИнформационныеБазы КАК СформированоОтчетовИнформационныеБазы
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаПоказателей КАК ТаблицаПоказателейОтбор
	|		ПО ТаблицаПоказателейОтбор.ПоказательМониторинга.Показатель = СформированоОтчетовИнформационныеБазы.Ссылка
	|		И НЕ ТаблицаПоказателейОтбор.ГруппаОбъектовКонтроля
	|
	|	ПО СформированоОтчетовИнформационныеБазы.ИнформационнаяБазаСсылка = Базы.ОбъектКонтроля
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаПоказателей КАК ТаблицаПоказателей
	|	ПО ТаблицаПоказателей.ПоказательМониторинга.Показатель = СформированоОтчетовИнформационныеБазы.Ссылка
	|
	|ГДЕ
...
Функция ПоискПоказателей(Знач СтрокаПоиска) Экспорт
	...
	Запрос.Текст = "
	|ВЫБРАТЬ
	|	""Отчеты/"" + ПараметрыИнформационныхБаз.ОбъектКонтроля.Наименование КАК Представление,
	...
	Пока Выборка.Следующий() Цикл
		...
		СтруктураРезультата.Вставить("ТипПоказателя", "СформированоОтчетов");
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	|	Описание
	|ИЗ
	|	Справочник.СформированоОтчетов
	|ГДЕ
	|	Ссылка = &Ссылка
	|";
