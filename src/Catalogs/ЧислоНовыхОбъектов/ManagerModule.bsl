...
Функция ПолучитьМассивСтруктурыПоказателей(ТипПоказателя, ИдентификаторНастройки, МассивСтруктурПоказателей, ТолькоВыбранные) Экспорт
	...
	Для Каждого СтруктураПоказатель Из МассивСтруктурПоказателей Цикл
		...
		Если ЗначениеЗаполнено(СтруктураПоказатель.Показатель) И СтруктураПоказатель.ТипПоказателя = "ЧислоНовыхОбъектов" Тогда
	...
	СтруктураДанныхГруппы.Вставить("Код", "Группы новых объектов");
	...
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ЧислоНовыхОбъектов.ИнформационныеБазы КАК ЧислоНовыхОбъектовИнформационныеБазы
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаПоказателей КАК ТаблицаПоказателейОтбор
	|		ПО ТаблицаПоказателейОтбор.ПоказательМониторинга.Показатель = ЧислоНовыхОбъектовИнформационныеБазы.Ссылка
	|		И НЕ ТаблицаПоказателейОтбор.ГруппаОбъектовКонтроля
	|
	|	ПО ЧислоНовыхОбъектовИнформационныеБазы.ИнформационнаяБазаСсылка = Базы.ОбъектКонтроля
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаПоказателей КАК ТаблицаПоказателей
	|	ПО ТаблицаПоказателей.ПоказательМониторинга.Показатель = ЧислоНовыхОбъектовИнформационныеБазы.Ссылка
	|
	|ГДЕ
...
Функция ПоискПоказателей(Знач СтрокаПоиска) Экспорт
	...
	Запрос.Текст = "
	|ВЫБРАТЬ
	|	""Новые объекты/"" + ПараметрыИнформационныхБаз.ОбъектКонтроля.Наименование КАК Представление,
	...
	Пока Выборка.Следующий() Цикл
		...
		СтруктураРезультата.Вставить("ТипПоказателя", "ЧислоНовыхОбъектов");
