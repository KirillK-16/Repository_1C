...
Функция ПолучитьДанные(
	...
	|	ОсновнаяТаблицаЗамеров.СозданоОбъектов + ОсновнаяТаблицаЗамеров.ИзмененоОбъектов КАК ТекущееЗначение,
	|	ВЫБОР
		...
		Если ВычислитьАгрегатныеЗначения Тогда
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(
					...
					Метаданные.Справочники.ЧислоНовыхОбъектов.МодульОбъекта,
					,
					Комментарий);
...
Функция ВсегоЗаПериод(Знач ДатаНачала, Знач ДатаОкончания) Экспорт
	...
	ЗапросТекст = "
	|ВЫБРАТЬ
	|	СУММА(ОсновнаяТаблицаЗамеров.СозданоОбъектов + ОсновнаяТаблицаЗамеров.ИзмененоОбъектов) КАК Всего
	|ИЗ
	|	РегистрСведений.СтатистикаПоИнформационнымБазам КАК ОсновнаяТаблицаЗамеров
	|ГДЕ
	|	%УсловиеНаБазы
...
Функция РасчитатьПоказатель(МенеджерВременныхТаблиц, Детализация = Неопределено) Экспорт
	...
	Пока ВыборкаТочки.Следующий() Цикл
		...
		Если Детализация = ТипДополненияПериодаКомпоновкиДанных.Минута
			...
			|		СУММА(ОсновнаяТаблицаЗамеров.СозданоОбъектов + ОсновнаяТаблицаЗамеров.ИзмененоОбъектов) КАК Максимальное,
			|		СУММА(ОсновнаяТаблицаЗамеров.СозданоОбъектов + ОсновнаяТаблицаЗамеров.ИзмененоОбъектов) КАК Минимальное,
			|		СУММА(ОсновнаяТаблицаЗамеров.СозданоОбъектов + ОсновнаяТаблицаЗамеров.ИзмененоОбъектов) КАК Значение
			|
			|	ИЗ
			|		РегистрСведений.СтатистикаПоИнформационнымБазам КАК ОсновнаяТаблицаЗамеров
			|
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ЧислоНовыхОбъектов.ИнформационныеБазы КАК СформированоОтчетовИнформационныеБазы
		...
		Иначе
			...
			|	ВЫБРАТЬ
			|		&ДатаТочкиТекущая КАК ДатаТочки,
			|		СУММА(ОсновнаяТаблицаЗамеров.СозданоОбъектовСумма + ОсновнаяТаблицаЗамеров.ИзмененоОбъектовСумма) КАК Максимальное,
			|		СУММА(ОсновнаяТаблицаЗамеров.СозданоОбъектовСумма + ОсновнаяТаблицаЗамеров.ИзмененоОбъектовСумма) КАК Минимальное,
			|		СУММА(ОсновнаяТаблицаЗамеров.СозданоОбъектовСумма + ОсновнаяТаблицаЗамеров.ИзмененоОбъектовСумма) КАК Значение
			|	ИЗ
