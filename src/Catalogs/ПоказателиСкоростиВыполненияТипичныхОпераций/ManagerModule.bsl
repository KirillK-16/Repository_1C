...
Функция ПолучитьМассивСтруктурыПоказателей(ТипПоказателя, ИдентификаторНастройки, МассивСтруктурПоказателей, ТолькоВыбранные) Экспорт
	...
	Для Каждого СтруктураПоказатель Из МассивСтруктурПоказателей Цикл
		...
		Если ЗначениеЗаполнено(СтруктураПоказатель.Показатель) И СтруктураПоказатель.ТипПоказателя = "ПоказателиСкоростиВыполненияТипичныхОпераций" Тогда
	...
	СтруктураДанныхГруппы.Вставить("Код", "Группы операций");
	...
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПоказателиСкоростиВыполненияТипичныхОпераций КАК ПоказателиСкоростиВыполненияТипичныхОпераций
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаПоказателей КАК ТаблицаПоказателейОтбор
	|		ПО ТаблицаПоказателейОтбор.ПоказательМониторинга.Показатель = ПоказателиСкоростиВыполненияТипичныхОпераций.Ссылка
	|		И НЕ ТаблицаПоказателейОтбор.ГруппаОбъектовКонтроля
	|
	|	ПО ПоказателиСкоростиВыполненияТипичныхОпераций.ИнформационнаяБаза = БазыОперации.ИнформационнаяБаза
	|	И ПоказателиСкоростиВыполненияТипичныхОпераций.ИдентификаторКлючевойОперации = БазыОперации.УникальныйИдентификатор
	|	И ПоказателиСкоростиВыполненияТипичныхОпераций.ТипСпискаПользователей = ЗНАЧЕНИЕ(Перечисление.ТипыСпискаПользователей.Все)
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаПоказателей КАК ТаблицаПоказателей
	|	ПО ТаблицаПоказателей.ПоказательМониторинга.Показатель = ПоказателиСкоростиВыполненияТипичныхОпераций.Ссылка
	|
	|УПОРЯДОЧИТЬ ПО
	|	БазаНаименование,
...
Функция ПоискПоказателей(Знач СтрокаПоиска) Экспорт
	...
	Запрос.Текст = "
	|ВЫБРАТЬ
	|	""КО/"" + БазыОперации.ИнформационнаяБаза.Наименование + ""/"" + БазыОперации.Имя КАК Представление,
	|	БазыОперации.ИнформационнаяБаза КАК ОбъектКонтроля,
	|	БазыОперации.УникальныйИдентификатор КАК КлючеваяОперация
	|ИЗ
	|	РегистрСведений.ОценкаПроизводительностиКлючевыеОперации КАК БазыОперации
	...
	Пока Выборка.Следующий() Цикл
		...
		СтруктураРезультата.Вставить("ТипПоказателя", "ПоказателиСкоростиВыполненияТипичныхОпераций");
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	|	Описание
	|ИЗ
	|	Справочник.ПоказателиСкоростиВыполненияТипичныхОпераций
	|ГДЕ
	|	Ссылка = &Ссылка
	|";
