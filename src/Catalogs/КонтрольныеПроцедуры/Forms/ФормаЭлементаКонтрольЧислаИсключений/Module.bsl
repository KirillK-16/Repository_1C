...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
    ...
    НастройкиРабочегоСервера = РегистрыСведений.ПараметрыРабочихСерверов.ПрочитатьНастройки(Объект.ОбъектКонтроля);
    Если НЕ ЗначениеЗаполнено(НастройкиРабочегоСервера.КаталогНастроекТЖ) ИЛИ НЕ ЗначениеЗаполнено(НастройкиРабочегоСервера.КаталогНастроекТЖСетевой) Тогда
        СообщениеПользователю = Новый СообщениеПользователю;
        СообщениеПользователю.Текст = "У рабочего сервера """ + Объект.ОбъектКонтроля + """ не настроены каталоги технологического журнала";
        СообщениеПользователю.КлючДанных = Объект.ОбъектКонтроля;
...
Процедура ЗаполнитьНастройки(ПоУмолчанию = Ложь)
	...
	Если ПоУмолчанию Тогда
		Если Строка(Объект.Владелец) = "Мониторинг системных ошибок" Тогда
...
Процедура СрокХраненияТЖПриИзменении(Элемент)
	НастройкиКлиент.НастроитьПодписьПериодаВЧасах(ЭтотОбъект.ПодписьСрокХраненияТЖ, СрокХраненияТЖ);
КонецПроцедуры
...
Функция КлючНастроек()
	Возврат "НастройкиФормыИсключенияВыводитьПредупреждение";
КонецФункции
