...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
    ...
    Если Объект.ОбъектКонтроля.ПлощадкаЭксплуатации.ПометкаУдаления ИЛИ Объект.ОбъектКонтроля.ПлощадкаЭксплуатации = Справочники.ПлощадкиЭксплуатации.Корзина() Тогда
        ...
        ЭтотОбъект.Элементы.ПараметрыИнцидентов.ТолькоПросмотр = Истина;
    КонецЕсли;
...
Процедура ЗаполнитьПараметрыИнцидентов()
	...
	Если Объект.Владелец = ВидКонтрольПроизводительности Тогда
		ЗаполнитьПараметрыИнцидентовКонтрольПроизводительности();
	КонецЕсли;

КонецПроцедуры

&НаСервере
Процедура ЗаполнитьПараметрыИнцидентовКонтрольПроизводительности()
	
	ПараметрыИнцидентаНетФайлов = Справочники.КонтрольныеПроцедуры.ПолучитьПараметрыИнцидентаКонтрольПроизводительности(Объект.Ссылка, Справочники.ТипыЗадачКонтрольПроизводительности.НетФайлов);
	НовСтрока = ЭтотОбъект.ПараметрыИнцидентов.Добавить();
	НовСтрока.ТипИнцидента = ПараметрыИнцидентаНетФайлов.НаименованиеТипаИнцидента;
	НовСтрока.КодИнцидента = ПараметрыИнцидентаНетФайлов.КодИнцидента;
	
	ПараметрыИнцидентаНеверныйФорматФайлов = Справочники.КонтрольныеПроцедуры.ПолучитьПараметрыИнцидентаКонтрольПроизводительности(Объект.Ссылка, Справочники.ТипыЗадачКонтрольПроизводительности.НеверныйФорматФайлов);
	НовСтрока = ЭтотОбъект.ПараметрыИнцидентов.Добавить();
	НовСтрока.ТипИнцидента = ПараметрыИнцидентаНеверныйФорматФайлов.НаименованиеТипаИнцидента;
	НовСтрока.КодИнцидента = ПараметрыИнцидентаНеверныйФорматФайлов.КодИнцидента;
    
    ПараметрыИнцидентаНеверныйФорматФайлов = Справочники.КонтрольныеПроцедуры.ПолучитьПараметрыИнцидентаКонтрольПроизводительности(Объект.Ссылка, Справочники.ТипыЗадачКонтрольПроизводительности.НеЗавершиласьПредыдущаяЗагрузкаФайлов);
	НовСтрока = ЭтотОбъект.ПараметрыИнцидентов.Добавить();
	НовСтрока.ТипИнцидента = ПараметрыИнцидентаНеверныйФорматФайлов.НаименованиеТипаИнцидента;
	НовСтрока.КодИнцидента = ПараметрыИнцидентаНеверныйФорматФайлов.КодИнцидента;
    
    ПараметрыИнцидентаНеверныйФорматФайлов = Справочники.КонтрольныеПроцедуры.ПолучитьПараметрыИнцидентаКонтрольПроизводительности(Объект.Ссылка, Справочники.ТипыЗадачКонтрольПроизводительности.ОшибкаЗагрузкиФайлов);
	НовСтрока = ЭтотОбъект.ПараметрыИнцидентов.Добавить();
	НовСтрока.ТипИнцидента = ПараметрыИнцидентаНеверныйФорматФайлов.НаименованиеТипаИнцидента;
	НовСтрока.КодИнцидента = ПараметрыИнцидентаНеверныйФорматФайлов.КодИнцидента;
...
Процедура ПараметрыИнцидентовВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	КлючЗаписи = СоздатьКлючЗаписиИнцидента(ТекДанные.КодИнцидента, ТекДанные.ТипИнцидента);
...
Функция СоздатьКлючЗаписиИнцидента(КодИнцидента, ТипИнцидента)
	...
	ЗначениеКлюча = Новый Структура("Инцидент, ТипИнцидента",КодИнцидента, ТипИнцидента);
...
Процедура ОбработкаНачалоВыбораЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если Истина = Ложь Тогда
		// Заглушка для прохождения проверки конфигурации,
		// т.к. ОбработкаНачалоВыбора подключается программно
		// для динамически созданных реквизитов
