...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
    ...
    Если
        ...
        ИЛИ Объект.ОбъектКонтроля.ПлощадкаЭксплуатации.ПометкаУдаления
        ИЛИ Объект.ОбъектКонтроля.ПлощадкаЭксплуатации = Справочники.ПлощадкиЭксплуатации.Корзина()
    Тогда
...
Процедура ЗаполнитьНастройки(ПоУмолчанию = Ложь)
    ...
    Настройки = РегистрыСведений.НастройкиКонтрольПамяти.ПрочитатьНастройки(Объект.Ссылка);
    Если Настройки.Мониторинг = 0 Тогда
        ЭтотОбъект.Мониторинг = "Отключить";
    ИначеЕсли Настройки.Мониторинг = 1 Тогда
        ЭтотОбъект.Мониторинг = "При превышении";
    ИначеЕсли Настройки.Мониторинг = 2 Тогда
        ЭтотОбъект.Мониторинг = "Постоянно";
    КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	КонецЕсли;
	ОбновитьОформлениеГруппыНастройкиСроковХраненияДанных();
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если АвтоматическиУдалятьЗаписиСеансов Тогда
		ПроверяемыеРеквизиты.Добавить("СрокХраненияДанных");
	КонецЕсли;
...
&НаКлиенте
Процедура ИсторияВыполненияПередНачаломИзменения(Элемент, Отказ)
	ОткрытьВыбранныйЭлемент();
...
&НаКлиенте
Процедура ОткрытьВыбранныйЭлемент()
...
Процедура ОбновитьСостояние()
	...
	ЭтотОбъект.Заголовок = "Мониторинг кластера" + СостояниеСтрока;
...
&НаКлиенте
Процедура ОбновитьОформлениеГруппыНастройкиСроковХраненияДанных()
	АвтоматическиУдалятьЗаписиСеансовПриИзменении(Неопределено);
	СрокХраненияДанныхПриИзменении(Неопределено);
КонецПроцедуры

&НаКлиенте
Процедура АвтоматическиУдалятьЗаписиСеансовПриИзменении(Элемент)
	Элементы.СрокХраненияСобранныхДанныхГруппа.Доступность = АвтоматическиУдалятьЗаписиСеансов;
КонецПроцедуры

&НаКлиенте
Процедура СрокХраненияДанныхПриИзменении(Элемент)
	НастройкиКлиент.НастроитьПодписьПериодаВЧасах(ЭтотОбъект.ПодписьСрокХраненияДанных, СрокХраненияДанных);
КонецПроцедуры
...
Процедура ЗаполнитьПараметрыИнцидентов()
	...
	Если Объект.Владелец = ВидКонтрольПотребленияПамяти Тогда
		ЗаполнитьПараметрыИнцидентовКонтрольПотребленияПамяти();
	КонецЕсли;

КонецПроцедуры

&НаСервере
Процедура ЗаполнитьПараметрыИнцидентовКонтрольПотребленияПамяти()
	
	ПараметрыИнцидентаНетДанныхПревышенПорогПотребленияПамяти = Справочники.КонтрольныеПроцедуры.ПолучитьПараметрыИнцидентаКонтрольПотребленияПамяти(Объект.Ссылка, Справочники.ТипыЗадачКонтрольПамяти.ПревышенПорогПотребленияПамяти);
	НовСтрока = ЭтотОбъект.ПараметрыИнцидентов.Добавить();
	НовСтрока.ТипИнцидента = ПараметрыИнцидентаНетДанныхПревышенПорогПотребленияПамяти.НаименованиеТипаИнцидента;
	НовСтрока.КодИнцидента = ПараметрыИнцидентаНетДанныхПревышенПорогПотребленияПамяти.КодИнцидента;
	
	ПараметрыИнцидентаНедоступенКластер = Справочники.КонтрольныеПроцедуры.ПолучитьПараметрыИнцидентаКонтрольПотребленияПамяти(Объект.Ссылка, Справочники.ТипыЗадачКонтрольПамяти.НедоступенКластер);
	НовСтрока = ЭтотОбъект.ПараметрыИнцидентов.Добавить();
	НовСтрока.ТипИнцидента = ПараметрыИнцидентаНедоступенКластер.НаименованиеТипаИнцидента;
	НовСтрока.КодИнцидента = ПараметрыИнцидентаНедоступенКластер.КодИнцидента;
	
	ПараметрыИнцидентаВосстановлениеРаботоспособоностиКонтрольнойПроцедуры = Справочники.КонтрольныеПроцедуры.ПолучитьПараметрыИнцидентаКонтрольПотребленияПамяти(Объект.Ссылка, Справочники.ТипыЗадач.ВосстановлениеРаботоспособоностиКонтрольнойПроцедуры);
	НовСтрока = ЭтотОбъект.ПараметрыИнцидентов.Добавить();
	НовСтрока.ТипИнцидента = ПараметрыИнцидентаВосстановлениеРаботоспособоностиКонтрольнойПроцедуры.НаименованиеТипаИнцидента;
	НовСтрока.КодИнцидента = ПараметрыИнцидентаВосстановлениеРаботоспособоностиКонтрольнойПроцедуры.КодИнцидента;
...
Процедура ПараметрыИнцидентовВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
    ...
    ТекДанные = ЭтотОбъект.Элементы.ПараметрыИнцидентов.ТекущиеДанные;
	КлючЗаписи = СоздатьКлючЗаписиИнцидента(ТекДанные.ТипИнцидента, ТекДанные.КодИнцидента);
...
&НаСервереБезКонтекста
Функция СоздатьКлючЗаписиИнцидента(ТипИнцидента, Инцидент)
    
    ЗначениеКлюча = Новый Структура("ТипИнцидента, Инцидент", ТипИнцидента, Инцидент);
