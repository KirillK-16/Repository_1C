...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекущаяБаза = Объект.ОбъектКонтроля;
	НастройкиОбъектаСловарь = РегистрыСведений.ПараметрыИнформационныхБаз.Получить(
		Новый Структура("ОбъектКонтроля", ТекущаяБаза)
	);
	КонтрольПроизводительностиПроцедура = Общий.КонтрольнаяПроцедура(ТекущаяБаза, "КонтрольПроизводительности");
	Элементы.УчитыватьПроизводительность.Доступность = КонтрольПроизводительностиПроцедура.НастройкаВыполнена;
	
	Кластер = НастройкиОбъектаСловарь.Кластер;
	Если ЗначениеЗаполнено(Кластер) Тогда
		
		КонтрольПамятиПроцедура = Общий.КонтрольнаяПроцедура(Кластер, "КонтрольПамяти");
		Если КонтрольПамятиПроцедура <> Неопределено Тогда
			Элементы.УчитыватьПотреблениеПамяти.Доступность = КонтрольПамятиПроцедура.НастройкаВыполнена;
		КонецЕсли;
		...
		УчитыватьБлокировкиДоступность = Ложь;
		УчитыватьИсключенияДоступность = Ложь;
		УчитыватьДампыДоступность = Ложь;
		
		Пока Выборка.Следующий() Цикл
			РабочийСервер = Выборка.РабочийСервер;
			Если НЕ УчитыватьИсключенияДоступность Тогда
				БлокировкиПроцедура = Общий.КонтрольнаяПроцедура(РабочийСервер, "КонтрольЧислаИсключений");
				УчитыватьИсключенияДоступность = БлокировкиПроцедура.НастройкаВыполнена;
			КонецЕсли;
			
			Если НЕ УчитыватьБлокировкиДоступность Тогда
				БлокировкиПроцедура = Общий.КонтрольнаяПроцедура(РабочийСервер, "АнализВызововКластера1С");
				УчитыватьБлокировкиДоступность = БлокировкиПроцедура.НастройкаВыполнена;
			КонецЕсли;
			
			Если НЕ УчитыватьДампыДоступность Тогда
				БлокировкиПроцедура = Общий.КонтрольнаяПроцедура(РабочийСервер, "СборДампов");
				УчитыватьДампыДоступность = БлокировкиПроцедура.НастройкаВыполнена;
			КонецЕсли;
			
			Если УчитыватьДампыДоступность И УчитыватьБлокировкиДоступность И УчитыватьИсключенияДоступность Тогда
				Прервать;
			КонецЕсли;
		КонецЦикла;
		
		Элементы.УчитыватьБлокировки.Доступность = УчитыватьБлокировкиДоступность;
		Элементы.УчитыватьДампы.Доступность = УчитыватьДампыДоступность;
		Элементы.УчитыватьИсключения.Доступность = УчитыватьИсключенияДоступность;
		
	Иначе
		
		Элементы.УчитыватьПотреблениеПамяти.Доступность = Ложь;
		Элементы.УчитыватьИсключения.Доступность = Ложь;
		Элементы.УчитыватьДампы.Доступность = Ложь;
		Элементы.УчитыватьБлокировки.Доступность = Ложь;
...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ Элементы.УчитыватьБлокировки.Доступность Тогда
		УчитыватьБлокировки = Ложь;
	КонецЕсли;
	Если НЕ Элементы.УчитыватьДампы.Доступность Тогда
		УчитыватьДампы = Ложь;
	КонецЕсли;
	Если НЕ Элементы.УчитыватьИсключения.Доступность Тогда
		УчитыватьИсключения = Ложь;
	КонецЕсли;
	Если НЕ Элементы.УчитыватьПотреблениеПамяти.Доступность Тогда
		УчитыватьПотреблениеПамяти = Ложь;
	КонецЕсли;
	Если НЕ Элементы.УчитыватьПроизводительность.Доступность Тогда
		УчитыватьПроизводительность = Ложь;
	КонецЕсли;
...
Процедура СтопЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		Режим = РежимДиалогаВопрос.ДаНет;
		ОписаниеОповещения = Новый ОписаниеОповещения("СтопЗадачиЗавершение", ЭтотОбъект);
		ПоказатьВопрос(ОписаниеОповещения, ТекстПредупреждения, Режим, 0);
	КонецЕсли;
...
&НаКлиенте
Процедура СтопЗадачиЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
