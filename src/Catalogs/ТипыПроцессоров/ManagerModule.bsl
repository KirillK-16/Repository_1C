...
Функция СоздатьЭлементПоНаименованию(Наименование) Экспорт
	...
	Если Результат.Пустой() Тогда
		Попытка
			...
			ЭлементБлокировки.УстановитьЗначение("Объект", "Справочник.ТипыПроцессоров." + Наименование);
			ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
			БлокировкаДанных.Заблокировать();
		Исключение
			ВызватьИсключение "Центр мониторинга. Ожидаемая управляемая блокировка при создании элемента справочника 'ТипыПроцессоров'";
		КонецПопытки;
		
		Результат = РезультатЗапросаПоНаименованию(Наименование);
		Если Результат.Пустой() Тогда
			НовыйЭлемент = Справочники.ТипыПроцессоров.СоздатьЭлемент();
