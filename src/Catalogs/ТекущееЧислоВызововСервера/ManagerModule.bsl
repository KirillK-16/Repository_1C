...
Функция ПолучитьМассивСтруктурыПоказателей(ТипПоказателя, ИдентификаторНастройки, МассивСтруктурПоказателей, ТолькоВыбранные) Экспорт
	...
	Для Каждого СтруктураПоказатель Из МассивСтруктурПоказателей Цикл
		...
		Если ЗначениеЗаполнено(СтруктураПоказатель.Показатель) И СтруктураПоказатель.ТипПоказателя = "ТекущееЧислоВызововСервера" Тогда
	...
	СтруктураДанныхГруппы.Вставить("Код", "Группы вызовов сервера");
	...
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ТекущееЧислоВызововСервера.ИнформационныеБазы КАК ТекущееЧислоВызововСервераИнформационныеБазы
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаПоказателей КАК ТаблицаПоказателейОтбор
	|		ПО ТаблицаПоказателейОтбор.ПоказательМониторинга.Показатель = ТекущееЧислоВызововСервераИнформационныеБазы.Ссылка
	|		И НЕ ТаблицаПоказателейОтбор.ГруппаОбъектовКонтроля
	|
	|	ПО ТекущееЧислоВызововСервераИнформационныеБазы.ИнформационнаяБазаСсылка = Базы.ОбъектКонтроля
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаПоказателей КАК ТаблицаПоказателей
	|	ПО ТаблицаПоказателей.ПоказательМониторинга.Показатель = ТекущееЧислоВызововСервераИнформационныеБазы.Ссылка
	|
	|ГДЕ
	...
	Пока ВыборкаКластер.Следующий() Цикл
		...
		СтруктураДанныхКластер.Вставить("ОбъектКонтроля", ВыборкаКластер.Кластер);
...
Функция ПоискПоказателей(Знач СтрокаПоиска) Экспорт
	...
	Запрос.Текст = "
	|ВЫБРАТЬ
	|	""Вызовы/"" + ПараметрыИнформационныхБаз.Кластер.Наименование + ""/"" + ПараметрыИнформационныхБаз.ОбъектКонтроля.Наименование КАК Представление,
	...
	Пока Выборка.Следующий() Цикл
		...
		СтруктураРезультата.Вставить("ТипПоказателя", "ТекущееЧислоВызововСервера");
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	|	Описание
	|ИЗ
	|	Справочник.ТекущееЧислоВызововСервера
	|ГДЕ
	|	Ссылка = &Ссылка
	|";
