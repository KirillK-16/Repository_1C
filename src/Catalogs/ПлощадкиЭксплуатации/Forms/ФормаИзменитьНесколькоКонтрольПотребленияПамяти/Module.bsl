...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
    ...
    Если ЭтотОбъект.Параметры.Свойство("ЭлементыПлощадки") Тогда
        ...
        МассивКонтрольПотребленияПамяти = ПолучитьКонтрольПотребленияПамяти(ЭтотОбъект.Параметры.ЭлементыПлощадки);
        КонтрольПотребленияПамяти = Новый СписокЗначений;
        КонтрольПотребленияПамяти.ЗагрузитьЗначения(МассивКонтрольПотребленияПамяти);
        
        //Если КонтрольПотребленияПамяти.Количество() > 0 Тогда
            ...
            НовЭлемент.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("КонтрольПотребленияПамяти");
            НовЭлемент.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке;
            НовЭлемент.ПравоеЗначение = КонтрольПотребленияПамяти;
        //КонецЕсли;
...
&НаСервере
Функция ПолучитьКонтрольПотребленияПамяти(ЭлементыПлощадкиМассив)
    
    Запрос = Новый Запрос;
    
    Запрос.Текст = "
    |ВЫБРАТЬ РАЗЛИЧНЫЕ
    |   ВЫРАЗИТЬ(ЕдиницаКонтроля КАК Справочник.КонтрольныеПроцедуры) КАК КонтрольПотребленияПамяти
    |ИЗ
    ...
    Запрос.УстановитьПараметр("ТипЭлемента", Перечисления.ТипЭлементаПлощадки.КонтрольПотребленияПамяти);
    
    Возврат Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("КонтрольПотребленияПамяти");
...
Процедура ЗаполнитьПараметрыЗаписи()
    ...
    ДобавитьПараметр("ПороговаяПамятьМБ", "Допустимый объем памяти, Мб", НастройкиПоУмолчанию["ПороговаяПамятьМБ"]);
    ДобавитьПараметр("СрокХраненияДанных", "Хранить данные, часов", НастройкиПоУмолчанию["СрокХраненияДанных"]);
    ДобавитьПараметр("АвтоматическиУдалятьЗаписиСеансов", "Автоматически удалять записи", НастройкиПоУмолчанию["АвтоматическиУдалятьЗаписиСеансов"]);
...
Процедура ЗаписатьИзменения()
    ...
    КонтрольПотребленияПамятиСписок = Новый СписокЗначений;
    Для Каждого ТекСтрока Из Список Цикл
        
        РегистрыСведений.НастройкиКонтрольПамяти.ЗаписатьНастройки(ТекСтрока.КонтрольПотребленияПамяти, НастройкиДляЗаписи);
        КонтрольПотребленияПамятиСписок.Добавить(ТекСтрока.КонтрольПотребленияПамяти);
        
        Если ЗаписьКонтрольнойПроцедуры Тогда
            
            КонтрольПотребленияПамятиОбъект = ТекСтрока.КонтрольПотребленияПамяти.ПолучитьОбъект();
            
            Если РасписаниеЗаписать <> Неопределено Тогда
                КонтрольПотребленияПамятиОбъект.Расписание  = Новый ХранилищеЗначения(РасписаниеЗаписать);
            КонецЕсли;
            
            Если СостояниеЗаписать <> Неопределено Тогда
                
                Если СостояниеЗаписать = "Выполняется" Тогда
                    КонтрольПотребленияПамятиОбъект.Выполнять = Истина;
                    КонтрольПотребленияПамятиОбъект.Пауза = Ложь;
                ИначеЕсли СостояниеЗаписать = "Пауза" Тогда
                    КонтрольПотребленияПамятиОбъект.Выполнять = Истина;
                    КонтрольПотребленияПамятиОбъект.Пауза = Истина;
                ИначеЕсли СостояниеЗаписать = "Отключен" Тогда
                    КонтрольПотребленияПамятиОбъект.Выполнять = Ложь;
                    КонтрольПотребленияПамятиОбъект.Пауза = Ложь;
                КонецЕсли;
                
            КонецЕсли;
            
            КонтрольПотребленияПамятиОбъект.Записать();
    ...
    НовЭлемент.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("КонтрольПотребленияПамяти");
    НовЭлемент.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке;
    НовЭлемент.ПравоеЗначение = КонтрольПотребленияПамятиСписок;
...
Процедура ИнициализироватьКомпоновщик()
    ...
    Поле.Поле = "КонтрольПотребленияПамяти";
    Поле.ПутьКДанным = "КонтрольПотребленияПамяти";
    Поле.Заголовок = "Контроль потребления памяти";
    ...
    Поле = НаборДанных.Поля.Добавить(Тип("ПолеНабораДанныхСхемыКомпоновкиДанных"));
    Поле.Поле = "ПороговаяПамятьМБ";
    Поле.ПутьКДанным = "ПороговаяПамятьМБ";
    Поле.Заголовок = "Допустимый объем памяти, Мб";
    Поле.ТипЗначения = Новый ОписаниеТипов("Число",,,Новый КвалификаторыЧисла(10,0));
    
    Поле = НаборДанных.Поля.Добавить(Тип("ПолеНабораДанныхСхемыКомпоновкиДанных"));
    Поле.Поле = "СрокХраненияДанных";
    Поле.ПутьКДанным = "СрокХраненияДанных";
    Поле.Заголовок = "Хранить данные, часов";
    Поле.ТипЗначения = Новый ОписаниеТипов("Число",,,Новый КвалификаторыЧисла(10,0));
    
    Поле = НаборДанных.Поля.Добавить(Тип("ПолеНабораДанныхСхемыКомпоновкиДанных"));
    Поле.Поле = "АвтоматическиУдалятьЗаписиСеансов";
    Поле.ПутьКДанным = "АвтоматическиУдалятьЗаписиСеансов";
    Поле.Заголовок = "Автоматически удалять записи";
    ...
    ВыбранноеПоле.Поле = Новый ПолеКомпоновкиДанных("КонтрольПотребленияПамяти");
...
Функция ТекстЗапроса()
    ...
    ТекстЗапроса = "
    |ВЫБРАТЬ
    |   НастройкиКонтрольПамяти.КонтрольнаяПроцедура КАК КонтрольПотребленияПамяти,
    ...
    Для Каждого ТекПараметр Из ПараметрыЗаписи Цикл
        ...
        Если ТекПараметр.Параметр <> "РасписаниеПредставление" И ТекПараметр.Параметр <> "Состояние" Тогда
            ТекстЗапроса = ТекстЗапроса + " НастройкиКонтрольПамяти." + ТекПараметр.Параметр + ",";
        КонецЕсли;
    ...
    |ИЗ
    |   РегистрСведений.НастройкиКонтрольПамяти КАК НастройкиКонтрольПамяти
    |ВНУТРЕННЕЕ СОЕДИНЕНИЕ
    |   Справочник.КонтрольныеПроцедуры КАК КонтрольныеПроцедуры
    |ПО
    |   КонтрольныеПроцедуры.Ссылка = НастройкиКонтрольПамяти.КонтрольнаяПроцедура
    |";
