...
Функция ПолучитьОбъектыЭлементовПлощадки()
    ...
    Возврат Справочники.ПлощадкиЭксплуатации.ПолучитьКластеры1С(ЭтотОбъект.Параметры.ЭлементыПлощадки);
...
Процедура ЗаполнитьПараметрыКонтроля()
    ...
    ДобавитьПараметр("ЗащищенноеСоединение", "Защищенное соединение", Неопределено, "Строка", СписокВыбора);
    
    ДобавитьПараметр("ИнтервалПерезапускаРабочихПроцессов", "Интервал перезапуска рабочих процессов, сек.", Неопределено, "Число");
    ДобавитьПараметр("ДопустимыйОбъемПамяти", "Допустимый объем памяти, КБ", Неопределено, "Число");
    ДобавитьПараметр("ИнтервалДопустимогоПревышенияОбъемаПамяти", "Интервал допустимого превышения объема памяти, сек.", Неопределено, "Число");
    ДобавитьПараметр("ДопустимоеОтклонениеОшибокСервера", "Допустимое отклонение ошибок сервера, %", Неопределено, "Число");
    
    СписокВыбора = Новый СписокЗначений;
    СписокВыбора.Добавить(Истина);
    СписокВыбора.Добавить(Ложь);
    ДобавитьПараметр("ПринудительноЗавершатьПроблемныеПроцессы", "Принудительно завершать проблемные процессы", Неопределено, "Булево", СписокВыбора);
    
    ДобавитьПараметр("ВыключенныеПроцессыОстанавливатьЧерез", "Выключенные процессы останавливать через, сек.", Неопределено, "Число");
    ДобавитьПараметр("УровеньОтказоустойчивости", "Уровень отказоустойчивости", Неопределено, "Число");
    
    СписокВыбора = Новый СписокЗначений;
    СписокВыбора.Добавить("Приоритет по производительности");
    СписокВыбора.Добавить("Приоритет по памяти");
    ДобавитьПараметр("РежимРаспределенияНагрузки", "Режим распределения нагрузки", Неопределено, "Строка", СписокВыбора);
...
Функция ТекстЗапроса()
    ...
    Для Каждого ТекПараметр Из ПараметрыКонтроля Цикл
        ...
        ТекстЗапроса = ТекстЗапроса + Символы.Таб + "ПараметрыКластеров." + ТекПараметр.Параметр + ",
...
Процедура УстановитьОграниченияПолейОтбора(НаборДанных)
    ...
    ЗащищенноеСоединение = НаборДанных.Поля.Найти("ЗащищенноеСоединение");
    ТипыЗащищенноеСоединение = Новый СписокЗначений;
    ТипыЗащищенноеСоединение.Добавить("выключено");
    ТипыЗащищенноеСоединение.Добавить("только соединение");
    ТипыЗащищенноеСоединение.Добавить("постоянно");
    ЗащищенноеСоединение.УстановитьДоступныеЗначения(ТипыЗащищенноеСоединение);
...
Процедура КонвертироватьДанные(ТЗнБуфер)
    ...
    ТЗнБуфер.Колонки.Добавить("ЗащищенноеСоединениеПредставление");
    ТЗнБуфер.Колонки.Добавить("РежимРаспределенияНагрузкиПредставление");
    
    Для Каждого ТекСтрока Из ТЗнБуфер Цикл
        
        Если ТекСтрока.ЗащищенноеСоединение = 0 Тогда
            ТекСтрока.ЗащищенноеСоединениеПредставление = "выключено"; 
        ИначеЕсли ТекСтрока.ЗащищенноеСоединение = 1 Тогда
            ТекСтрока.ЗащищенноеСоединениеПредставление = "только соединение"; 
        ИначеЕсли ТекСтрока.ЗащищенноеСоединение = 2 Тогда
            ТекСтрока.ЗащищенноеСоединениеПредставление = "постоянно"; 
        КонецЕсли;
        
        Если ТекСтрока.РежимРаспределенияНагрузки = 0 Тогда
            ТекСтрока.РежимРаспределенияНагрузкиПредставление = "Приоритет по производительности";
        ИначеЕсли ТекСтрока.РежимРаспределенияНагрузки = 1 Тогда
            ТекСтрока.РежимРаспределенияНагрузкиПредставление = "Приоритет по памяти";
        КонецЕсли;
                
    КонецЦикла;
    
    ТЗнБуфер.Колонки.Удалить("ЗащищенноеСоединение");
    ТЗнБуфер.Колонки.Добавить("ЗащищенноеСоединение");
    ТЗнБуфер.Колонки.Удалить("РежимРаспределенияНагрузки");
    ТЗнБуфер.Колонки.Добавить("РежимРаспределенияНагрузки");
    
    ТЗнБуфер.ЗагрузитьКолонку(ТЗнБуфер.ВыгрузитьКолонку("ЗащищенноеСоединениеПредставление"), "ЗащищенноеСоединение");
    ТЗнБуфер.ЗагрузитьКолонку(ТЗнБуфер.ВыгрузитьКолонку("РежимРаспределенияНагрузкиПредставление"), "РежимРаспределенияНагрузки");
    
    ТЗнБуфер.Колонки.Удалить("ЗащищенноеСоединениеПредставление");
    ТЗнБуфер.Колонки.Удалить("РежимРаспределенияНагрузкиПредставление");
...
Процедура ЗаписатьИзмененияПереопределяемая()
    ...
    ЗащищенноеСоединениеКонвертация = Новый Соответствие;
    ЗащищенноеСоединениеКонвертация.Вставить("выключено", 0);
    ЗащищенноеСоединениеКонвертация.Вставить("только соединение", 1);
    ЗащищенноеСоединениеКонвертация.Вставить("постоянно", 2);
    
    РежимРаспределенияНагрузкиКонвертация = Новый Соответствие;
    РежимРаспределенияНагрузкиКонвертация.Вставить("Приоритет по производительности", 0);
    РежимРаспределенияНагрузкиКонвертация.Вставить("Приоритет по памяти", 1);
    ...
    Для Каждого ТекКолонка Из ТЗнБуфер.Колонки Цикл
        ...
        Если СтрНачинаетсяС(ТекКолонка.Имя, "_") Тогда
            ...
            ТекКластер = ЭтотОбъект.Ссылки[ИндексСсылки];
            
            ПараметрыКонтроляКластера = РегистрыСведений.ПараметрыКластеров.ПрочитатьНастройки(ТекКластер.Значение);
            НастройкиКонтроляКластера = РегистрыСведений.КонтрольПараметровОбъектов.ПрочитатьНастройки(ТекКластер.Значение);
            ...
            Для Каждого ТекСтрока Из ЭтотОбъект.Список Цикл
                ...
                Если ТекСтрока.Редактировать Тогда
                    ...
                    Если ЭтоКонтроль Тогда
                        ...
                        НастройкиКонтроляКластера[СтрЗаменить(Параметр, "Контроль", "")] = ТекСтрока[ТекКолонка.Имя]; 
                        
                    Иначе
                        
                        Если Параметр = "ЗащищенноеСоединение" Тогда
                            ПараметрыКонтроляКластера[ТекСтрока.Параметр] = ЗащищенноеСоединениеКонвертация[ТекСтрока[ТекКолонка.Имя]];
                        ИначеЕсли Параметр = "РежимРаспределенияНагрузки"  Тогда
                            ПараметрыКонтроляКластера[ТекСтрока.Параметр] = РежимРаспределенияНагрузкиКонвертация[ТекСтрока[ТекКолонка.Имя]];
                        Иначе
                            ПараметрыКонтроляКластера[ТекСтрока.Параметр] = ТекСтрока[ТекКолонка.Имя];
                        КонецЕсли;
            ...
            РегистрыСведений.ПараметрыКластеров.ЗаписатьНастройки(ТекКластер.Значение, ПараметрыКонтроляКластера);
            РегистрыСведений.КонтрольПараметровОбъектов.ЗаписатьНастройки(ТекКластер.Значение, НастройкиКонтроляКластера);
    ...
    Для Каждого ТекПараметр Из ЭтотОбъект.ПараметрыКонтроля Цикл
        ...
        ИначеЕсли НовСтрока.ТипСтр = "Строка" Тогда
            ...
            Если ТекПараметр.Параметр = "ЗащищенноеСоединение" Тогда
                НовСтрока.ПараметрБыстроеРедактирование = "выключено";
            ИначеЕсли ТекПараметр.Параметр = "РежимРаспределенияНагрузки" Тогда
                НовСтрока.ПараметрБыстроеРедактирование = "Приоритет по производительности";
            КонецЕсли;
