...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
    ...
    Если ЭтотОбъект.Параметры.Свойство("ЭлементыПлощадки") Тогда
        ...
        МассивКонтрольПроизводительности = ПолучитьКонтрольПроизводительности(ЭтотОбъект.Параметры.ЭлементыПлощадки);
        КонтрольПроизводительностиСписок = Новый СписокЗначений;
        КонтрольПроизводительностиСписок.ЗагрузитьЗначения(МассивКонтрольПроизводительности);
        
        //Если КонтрольПроизводительностиСписок.Количество() > 0 Тогда
            ...
            НовЭлемент.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("КонтрольПроизводительности");
            НовЭлемент.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке;
            НовЭлемент.ПравоеЗначение = КонтрольПроизводительностиСписок;
        //КонецЕсли;
...
&НаСервере
Функция ПолучитьКонтрольПроизводительности(ЭлементыПлощадкиМассив)
    
    Запрос = Новый Запрос;
    
    Запрос.Текст = "
    |ВЫБРАТЬ РАЗЛИЧНЫЕ
    |   ВЫРАЗИТЬ(ЕдиницаКонтроля КАК Справочник.КонтрольныеПроцедуры) КАК КонтрольПроизводительности
    |ИЗ
    ...
    Запрос.УстановитьПараметр("ТипЭлемента", Перечисления.ТипЭлементаПлощадки.КонтрольПроизводительности);
    
    Возврат Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("КонтрольПроизводительности");
...
Процедура ЗаполнитьПараметрыЗаписи()
    ...
    ДобавитьПараметр("КаталогФайловОценкиПроизводительности", "Каталог файлов оценки производительности", НастройкиПоУмолчанию["КаталогФайловОценкиПроизводительности"]);
...
Процедура ЗаписатьИзменения()
    ...
    Для Каждого ТекСтрока Из Список Цикл
        ...
        РегистрыСведений.НастройкиКонтрольПроизводительности.ЗаписатьНастройки(ТекСтрока.КонтрольПроизводительности, НастройкиДляЗаписи);
        КонтрольПроизводительностиСписок.Добавить(ТекСтрока.КонтрольПроизводительности);
        
        Если ЗаписьКонтрольнойПроцедуры Тогда
            
            КонтрольПроизводительностиОбъект = ТекСтрока.КонтрольПроизводительности.ПолучитьОбъект();
            
            Если РасписаниеЗаписать <> Неопределено Тогда
                КонтрольПроизводительностиОбъект.Расписание  = Новый ХранилищеЗначения(РасписаниеЗаписать);
            КонецЕсли;
            
            Если СостояниеЗаписать <> Неопределено Тогда
                
                Если СостояниеЗаписать = "Выполняется" Тогда
                    КонтрольПроизводительностиОбъект.Выполнять = Истина;
                    КонтрольПроизводительностиОбъект.Пауза = Ложь;
                ИначеЕсли СостояниеЗаписать = "Пауза" Тогда
                    КонтрольПроизводительностиОбъект.Выполнять = Истина;
                    КонтрольПроизводительностиОбъект.Пауза = Истина;
                ИначеЕсли СостояниеЗаписать = "Отключен" Тогда
                    КонтрольПроизводительностиОбъект.Выполнять = Ложь;
                    КонтрольПроизводительностиОбъект.Пауза = Ложь;
                КонецЕсли;
                
            КонецЕсли;
            
            КонтрольПроизводительностиОбъект.Записать();
...
Процедура ИнициализироватьКомпоновщик()
    ...
    Поле.Поле = "КонтрольПроизводительности";
    Поле.ПутьКДанным = "КонтрольПроизводительности";
    Поле.Заголовок = "Контроль производительности";
    ...
    Поле = НаборДанных.Поля.Добавить(Тип("ПолеНабораДанныхСхемыКомпоновкиДанных"));
    Поле.Поле = "КаталогФайловОценкиПроизводительности";
    Поле.ПутьКДанным = "КаталогФайловОценкиПроизводительности";
    Поле.Заголовок = "Каталог файлов оценки производительности";
    Поле.ТипЗначения = Новый ОписаниеТипов("Строка",,,,Новый КвалификаторыСтроки(1000));
    ...
    ВыбранноеПоле.Поле = Новый ПолеКомпоновкиДанных("КонтрольПроизводительности");
...
Функция ТекстЗапроса()
    ...
    ТекстЗапроса = "
    |ВЫБРАТЬ
    |   НастройкиКонтрольПроизводительности.КонтрольнаяПроцедура КАК КонтрольПроизводительности,
    ...
    Для Каждого ТекПараметр Из ПараметрыЗаписи Цикл
        ...
        Если ТекПараметр.Параметр <> "РасписаниеПредставление" И ТекПараметр.Параметр <> "Состояние" Тогда
            ТекстЗапроса = ТекстЗапроса + " НастройкиКонтрольПроизводительности." + ТекПараметр.Параметр + ",";
        КонецЕсли;
    ...
    |ПО
    |   КонтрольныеПроцедуры.Ссылка = НастройкиКонтрольПроизводительности.КонтрольнаяПроцедура
    |";
