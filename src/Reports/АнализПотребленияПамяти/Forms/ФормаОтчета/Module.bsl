...
Процедура СформироватьНаСервере()
    ...
    Запрос = Новый Запрос;
    Запрос.Текст = ТекстЗапроса(Отчет.Кластер, Отчет.РабочийСервер);
    ...
    Запрос.УстановитьПараметр("РабочийСервер", Отчет.РабочийСервер);
...
&НаСервере
Функция ТекстЗапроса(Кластер, РабочийСервер)
	...
	|   ОбъектыКластера
    |ИЗ
	|   РегистрСведений.ПараметрыРабочихСерверов КАК РабочиеСерверы
    |ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|   Кластеры
    |ПО
	|   РабочиеСерверы.Кластер = Кластеры.Ссылка
    |{ГДЕ
	|   РабочиеСерверы.ОбъектКонтроля КАК РабочийСервер}
    |ИНДЕКСИРОВАТЬ ПО
	|   Кластер,
	|   РабочийСервер
    |;
    ...
    |       Процессы.Сервер,
    |       Процессы.Процесс,
    |       ВЫРАЗИТЬ(МАКСИМУМ(Процессы.Память/1024) КАК ЧИСЛО(15,3)) КАК Память
    |   ИЗ
    |       ОбъектыКластера КАК ОбъектыКластера
    |   ВНУТРЕННЕЕ СОЕДИНЕНИЕ
    |       РегистрСведений.МониторингПроцессов КАК Процессы
    |   ПО
	|       Процессы.Кластер = ОбъектыКластера.Кластер
	|       И Процессы.Сервер = ОбъектыКластера.РабочийСервер
    ...
    Если ЗначениеЗаполнено(РабочийСервер) Тогда
        ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "{ГДЕ
	    |   РабочиеСерверы.ОбъектКонтроля КАК РабочийСервер}", "ГДЕ
	    |   РабочиеСерверы.ОбъектКонтроля = &РабочийСервер");
    Иначе
        ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "{ГДЕ
	    |   РабочиеСерверы.ОбъектКонтроля КАК РабочийСервер}", "");
    КонецЕсли;
...
Функция ИсточникДанных(РезультатЗапроса)
    ...
    Пока Выборка.Следующий() Цикл
        ...
        Если ЗначениеЗаполнено(Выборка.Кластер) И ЗначениеЗаполнено(Выборка.Сервер) И ЗначениеЗаполнено(Выборка.Процесс) Тогда
            ...
            СтрокаИсточника[ИмяКолонки] = Выборка.Память;
...
Функция Сеансы(Период, ОтборСерии)
    ...
    |   Сеансы.Процесс,
    |   ВЫРАЗИТЬ(Сеансы.ПамятьТекущая/1024 КАК ЧИСЛО(15, 0)) КАК ПамятьТекущая
    |ИЗ
...
Функция СерииУникальные(РезультатЗапроса)
    ...
    Пока Выборка.Следующий() Цикл
        ...
        Если СерияУникальная = Неопределено Тогда
            ...
            СерияУникальная = Новый Структура("Кластер, База, Сервер, Сеанс, Процесс, ИдентификаторСеанса, Приложение, Пользователь, ПамятьТекущая, ПамятьВыделено");
            ЗаполнитьЗначенияСвойств(СерияУникальная, Выборка);
            СерияУникальная.ПамятьВыделено = 0;
            СерииУникальные.Вставить(Выборка.ИдентификаторСеанса, СерияУникальная);
            
        Иначе
            
            ПамятьПрирост = Выборка.ПамятьТекущая - СерияУникальная.ПамятьТекущая;
            СерияУникальная.ПамятьТекущая = Выборка.ПамятьТекущая;
            
            Если ПамятьПрирост > 0 Тогда
                СерияУникальная.ПамятьВыделено = СерияУникальная.ПамятьВыделено + ПамятьПрирост;
            КонецЕсли;
...
Функция ИсточникДанныхСерииУникальные(СерииУникальные)
    ...
    ТЗнИсточникДанных.Колонки.Добавить("Память");
    ТЗнИсточникДанных.Индексы.Добавить("Память");
    
    Для Каждого СерияУникальная Из СерииУникальные Цикл
        
        Если СерияУникальная.Значение.ПамятьВыделено > 0 Тогда
            ...
            НовСерия.Память = СерияУникальная.Значение.ПамятьВыделено;
    ...
    ТЗнИсточникДанных.Сортировать("Память Убыв");
...
Процедура УстановитьЗначениеСерииСеансов(Диаграмма, СерииПараметры)
    ...
    Для Каждого Серия Из ЭтотОбъект.ДиаграммаСеансов.Серии Цикл
        ...
        Для Каждого Точка Из ЭтотОбъект.ДиаграммаСеансов.Точки Цикл
            ...
            ЭтотОбъект.ДиаграммаСеансов.УстановитьЗначение(Точка, Серия, Серия.Значение.ПамятьВыделено,,Подсказка);
...
Функция ПодсказкаСеансов(Точка, Серия)
    ...
    Содержимое.Добавить(Новый ФорматированнаяСтрока(Формат(Серия.Значение.ПамятьВыделено, "ЧГ=3,0") + " кБ" + Символы.ПС, Новый Шрифт(,12,Истина,,Истина), WebЦвета.Синий));
...
Функция ИсточникДанныхСеансыПодробно(РезультатЗапроса)
    ...
    Пока Выборка.Следующий() Цикл
        ...
        СтрокаИсточника[ИмяКолонки] = Выборка.ПамятьТекущая;
